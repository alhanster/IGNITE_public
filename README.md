...
- R 4.4.1, packages pinned in `R-requirements.txt`
...
| `write_baseline.sh` | regenerates `final_plots.sha256` from a clean clone |
...
Every input `make tables` needs is committed under `data/`; the offline build works from a clean clone. Per-file provenance for the inputs is in `DATA_AVAILABILITY.md`; for the derived tables, in `figure_data/PROVENANCE.md`.
...
Third-party data redistributed here, each to be cited independently: **Open Targets** (release 26.06), **HGNC**, **GWAS Catalog**, **STRING** (v12.0), **Gene Ontology / UniProt-GOA**, **gnomAD** (v4.1.0), **IUIS** IEI gene list, the **GPS** dataset, **ChEMBL** and **ClinicalTrials.gov**, and the perturb-seq resource of Zhu, Dann et al. (2025). See `DATA_AVAILABILITY.md`.
...
| `DATA_AVAILABILITY.md` | every external input, where it lives, what is committed, redistribution terms |
...
Cite the manuscript this repository accompanies, the upstream perturb-seq resource (Zhu, Dann et al., 2025, released via the CZI Virtual Cells Platform), and the third-party data sources named in `DATA_AVAILABILITY.md`, each under its own terms.
