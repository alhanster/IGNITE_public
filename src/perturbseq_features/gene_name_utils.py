...
Functions used across the scripts in this directory for gene-name
formatting checks and HGNC symbol handling.

The HGNC reference is a pinned local TSV; rename maps are derived from it
dynamically. load_hgnc refuses to fetch the current release unless asked to:
symbols are renamed between releases, and a silent fetch re-keys every gene
join downstream of it.

Functions
    load_hgnc(path=None, refresh=False, allow_download=False)
        -> dict(approved, prev2app, alias, n_records, release)
...
_SOURCES = Path(__file__).resolve().parents[2] / "data" / "perturbseq" / "sources"
...
def _main(argv: list[str]) -> None:
    if not argv:
        sys.exit("usage: python gene_name_utils.py <csv> [gene_col] [--download]")
