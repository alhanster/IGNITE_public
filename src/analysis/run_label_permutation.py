...
Tests whether the fitting procedure could produce an AUC ladder as strong as the observed
one from random labels. Each permutation reassigns the positive label to a random set of
len(P) genes drawn from the scorable pool (real positives plus unlabeled genes), reruns the
full cross-validation pipeline with the same base learner and feature groups used for the
observed ladder, and records the resulting ladder. Trial-heldout genes remain excluded from
the pool and are never assigned the positive label. Features are unchanged. The empirical
p-value per rung is the fraction of permutations whose null AUC meets or exceeds the
observed AUC.

Each permutation seeds from RandomState(10_000 + i), so results do not depend on thread
count; only wall-clock time scales with --jobs. Running 1000 permutations takes roughly 2-3
hours at --jobs 11. Use the opt-in `make permutation-null` target, which checkpoints every
25 permutations and resumes.
