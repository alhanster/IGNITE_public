...
Two PU-bagging arms are fit on identical labels and hyperparameters, differing
only in feature set: a genetics-only arm (lof.oe_ci.upper, mis.z_score, IEI,
gwas_score, gene_burden_score) and a full arm adding a 19-feature functional-genomics block. Both
arms score every gene; scores are converted to rank percentiles over the same
...
mark genes the genetic prior alone ranked higher. Both arms are fit once at
pum.SEED and T=pum.T_BAG, the production configuration behind the committed
atlas scores.

See REPRODUCIBILITY.md, Build-time assertions inside analysis scripts, for the verification procedure
and documented deviations from the source analysis.
"""
import json
import os
import sys
...
HI, LO = 0.75, 0.25               # symmetric core-set rule (see module docstring)
