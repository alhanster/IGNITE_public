#!/usr/bin/env python3
"""Records steps that skipped because an uncommitted input was absent.

A skipped step leaves its committed figure_data/ tables in place, so the build completes and
`make verify-tables` passes. That is the intended behaviour on a clean clone, but it means
those tables were copied rather than re-derived. Each skip is appended here so `make tables`
can list them at the end of the run, and so the reproduction boundary is visible in the build
log rather than only in the middle of it.

The ledger is plain TSV at outputs/_skipped.tsv, removed at the start of `make tables`.
"""
import os
