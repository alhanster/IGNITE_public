...
`src/analysis/_version_guard.py` runs `check_pins()` at import of `pu_target_model.py`, as step 1
...
`audit_coverage()` in `src/analysis/_version_guard.py` checks that every script constructing an
...
- Checks that tie a table to the committed run print a warning instead of stopping the build:
  ...
  `src/analysis/_platform.py` holds the one platform test these checks share.
...
few clones. It is regenerated by `make knn-signatures` (`src/analysis/build_knn_signatures.py`),
...
```bash
PY=.venv/bin/python RSCRIPT=Rscript tools/run_step.sh py src/analysis/build_discordance.py
...
```bash
PYTHONPATH=src .venv/bin/python src/analysis/build_specificity_matrix.py
