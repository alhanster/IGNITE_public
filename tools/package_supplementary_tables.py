...
Called by make final-outputs (writes to final_outputs/supplementary_tables/, the default) and by make supplementary (writes to supplementary/tables/, via --out-dir). Reads tools/supplementary_tables.tsv, the manifest that sets each table's number and display name, and copies each source file under its manuscript name. The generated README's column dictionary is read from each output file's actual header, not from the manifest.
...
def describe(path):
    """Column dictionary read from the file's actual header, never from the manifest, so mismatches are visible rather than assumed."""
    ext = os.path.splitext(path)[1].lower()
    if ext == ".json":
        with open(path) as fh:
            obj = json.load(fh)
        keys = sorted(obj) if isinstance(obj, dict) else []
        return f"JSON object, {len(keys)} top-level keys", keys
    with open(path, newline="") as fh:
        reader = csv.reader(fh)
        header = next(reader, [])
        n = sum(1 for _ in reader)
    return f"{n:,} data rows x {len(header)} columns", header
...
def main():
    ...
    lines = ["# Supplementary tables", "",
             "Generated by `tools/package_supplementary_tables.py` from "
             "`tools/supplementary_tables.tsv`. Do not edit: `make final-outputs` overwrites it.",
             "",
             PLATFORM_STATEMENT,
             "",
             "Every column dictionary below is read from the packaged file's own header, so it "
    ...
    for r in sorted(rows, key=lambda r: r["number"]):
        ...
        lines += [f"## Supp Table {r['number']} — {r['title']}", "", f"- **File:** `{name}`"]

        if multi:
            paths = {s: os.path.join(SUPP, f) for s, f in r["sheets"].items()}
            write_workbook(paths, dest)
            lines += [f"- **Sheets:** {len(paths)}", "", r["description"], ""]
            for sheet, path in paths.items():
                shape, cols = describe(path)
                lines += [f"**{sheet}** — `figure_data/{os.path.basename(path)}`, {shape}", ""]
                if cols:
                    lines += ["| # | Column |", "|---:|---|"]
                    lines += [f"| {i} | `{c}` |" for i, c in enumerate(cols, 1)]
                    lines += [""]
            shape = f"{len(paths)} sheets"
        else:
            src = os.path.join(SUPP, r["source"])
            shutil.copyfile(src, dest)
            shape, cols = describe(src)
            lines += [f"- **Built from:** `figure_data/{r['source']}`",
                      f"- **Shape:** {shape}", "", r["description"], ""]
            if cols:
                lines += ["| # | Column |", "|---:|---|"]
                lines += [f"| {i} | `{c}` |" for i, c in enumerate(cols, 1)]
                lines += [""]

        print(f"  Supp Table {r['number']}: -> {name}  ({shape})")
